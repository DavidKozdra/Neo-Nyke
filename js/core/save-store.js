// save-store.js — IndexedDB/localStorage save store factory.
...
export function createSaveStore() {
  ...
  function createFallbackApi(key) {
    ...
    try {
      return new SaveApiCtor({ driver: createLocalStorageDriver(localStorage), key: localPrefix + key });
    } catch {
      return null;
    }
  }
  ...
  function openDb() {
    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        if (!request.result.objectStoreNames.contains('achievements')) request.result.createObjectStore('achievements', { keyPath: 'id' });
      };
  ...
  return {
    ...
    async get(key) {
      ...
      try { return await idbGet(key); } catch { this.kind = 'LOCAL ONLY'; return fallback.get(key); }
    },
    async put(key, value) {
      if (!idb) return fallback.put(key, value);
      try { return await idbPut(key, value); } catch { this.kind = 'LOCAL ONLY'; return fallback.put(key, value); }
    },
    async delete(key) {
      if (!idb) return fallback.delete(key);
      try { return await idbDelete(key); } catch { this.kind = 'LOCAL ONLY'; return fallback.delete(key); }
    },
  };
}
