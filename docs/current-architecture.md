...
- The initial room Durable Object is authoritative for lobby membership, ready state, a 20 Hz headless movement simulation, input validation, a seeded 8–10-room floor layout, and snapshots. Browsers render the shared start chamber and real character sprites, sample normal controls, predict the local player, and interpolate remote players. It uses the shared protocol and simulation modules rather than accepting client positions. It is a playable milestone-A slice, not yet the complete campaign authority.
