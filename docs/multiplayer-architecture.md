...
Room creation explicitly chooses `public` or `private` visibility and a `shared` or `vote` party-pause rule. Private is the transport/server default for older callers; the current menu presents a public-by-default earth/lock toggle. The pause rule remains an authority-backed toggle in the waiting lobby; changing it broadcasts the new rule and clears ready confirmations before the match can start. Shared pause lets any connected player pause or resume the authority, while vote pause requires a strict majority of currently connected players for either transition. Public waiting rooms are placed in a short-lived KV discovery index and `GET /api/multiplayer/rooms` rechecks each bounded candidate against its room Durable Object before returning joinable results, including the authoritative pause rule. Private rooms never enter that index and remain accessible only by invite code or invite link.
...
The room uses the Durable Object WebSocket Hibernation API. Lobby, authority-paused, and empty rooms keep no simulation interval, so the object can be evicted while accepted sockets stay connected; a 20 Hz interval exists only while an unpaused match is running and at least one peer is present. A resume request wakes the room through its socket and restarts the interval after the authority accepts the shared action or vote. Serializable socket attachments plus the persisted authority/runtime checkpoint rebuild peer ownership, input sequencing, reconnect reservations, pause state, and simulation state after a cold wake. Empty rooms receive a cleanup alarm and expire after ten minutes.
...
Normal publication targets 10 Hz. A peer whose oldest unacknowledged view becomes stale degrades independently to 5 Hz and then 2.5 Hz, while other peers remain unaffected. Recovery uses hysteresis and returns through 5 Hz before 10 Hz, preventing one ACK from immediately reflooding the same slow link. Deltas are calculated from that peer's acknowledged baseline, and clients retain enough baseline history to rebase concurrent views without a sequence-gap resync. Current regression gates require under 80 KiB/player/second in the 100-player profiler, under 180 ms packed snapshot-age p95 at 100 ms RTT/1 Mbps, under 300 ms at 160 ms RTT/250 kbps, and bounded snapshot gaps/correction blending with 60 ms jitter, 8% unreliable loss, and duplicate delivery. Continuous-motion presentation is sampled at 60 fps under both degraded remote cadences and for the local hero between 20 Hz authority/input samples; overlapping correction blends preserve the currently displayed position so a newer snapshot cannot reintroduce a visible step.

The browser capacity proof runs 40 isolated clients across ten four-player Durable Object rooms and requires every session to join, ready, render, receive authority ticks, move, and converge without page errors. Its swarm movement probe drives the browser session/WebSocket seam directly because Chromium intentionally marks most same-process pages hidden and the production input sampler correctly suppresses hidden-tab input. The active-page two-client smoke separately verifies keyboard prediction and presentation. Socket teardown is loss-tolerant: a close event racing the 20 Hz snapshot loop records a dropped stale-recipient delivery instead of aborting the room tick for the remaining players.
...
The pre-game co-op lobby embeds the same authority-routed party chat used during a run, so players can coordinate hero choices and the message history continues into gameplay. Pressing T focuses chat in either context. Messages are sanitized, limited to 180 characters, rate-limited by the authority, and attributed from server-owned player identity rather than client-supplied names. Opening chat during gameplay releases gameplay input and pointer lock until the form closes.

The normal multiplayer screen uses the campaign HUD rather than a separate always-on debug HUD. Escape submits the room's pause intent. In shared mode the authority pauses immediately for every player; in vote mode play continues while the vote banner shows progress, then pauses when the connected-player majority is reached. Resume is governed by the same room rule. While authority-paused, simulation ticks and gameplay commands stop, but WebSocket control, chat, reconnect, and pause-vote messages remain live. `F8` opens the opt-in diagnostics overlay without replacing gameplay presentation.
