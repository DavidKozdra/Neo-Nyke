...
- The initial room Durable Object is authoritative for lobby membership, validated character choice, ready state, a 20 Hz headless movement simulation, input validation, a seeded 8–10-room floor layout, connected-door collision, party room transitions, visited rooms, and snapshots. Browsers render the shared current chamber and real selected character sprites, sample normal controls, predict the local player, and interpolate remote players. It uses the shared protocol and simulation modules rather than accepting client positions. It is a playable exploration slice, not yet the complete campaign authority.
