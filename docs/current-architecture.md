...
- The room Durable Object is authoritative for lobby membership, validated character choice, ready state, a 20 Hz headless simulation, input/action validation, seeded 8–10-room floors and encounters, connected-door collision, enemy AI, player projectiles, damage/death, exactly-once drops/currency, party room transitions, visited rooms, gameplay events, and snapshots. It uses hibernating WebSockets while waiting, runs its timer only for active matches, checkpoints running state at a bounded cadence and lifecycle boundaries, expires abandoned rooms by alarm, and publishes entity deltas with periodic full corrections. Browsers render the shared chamber with real character/enemy sprites and local visual effects, sample normal controls, predict the local player, and interpolate remote entities. It is a playable combat/exploration slice, not yet the complete campaign authority.
...
- `wrangler.toml` serves `dist/` static assets, routes `/api/*` to the Worker, and binds the `MULTIPLAYER_ROOMS` Durable Object namespace. The browser WebSocket transport probes the current origin first so Wrangler's automatic fallback ports (such as 8789) work, then tries port 8787 when the static site is served separately on localhost.
